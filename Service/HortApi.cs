...
namespace HortBot.Service
{
    public class HortApi
    {
        ...
        public HortApi(Config config)
        {
            ...
            SaveLoginCookieAsync();
        }

        public void SaveLoginCookieAsync(bool forceNewCookie = false)
        {
            try
            {
                ...
                else
                {
                    ...
                    Task<HttpResponseMessage> post = loginClient.PostAsync(loginClient.BaseAddress, data);
                    post.Wait();

                    Console.WriteLine($"{nameof(SaveLoginCookieAsync)}: " + post.Result);
